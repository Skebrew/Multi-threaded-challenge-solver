...
def test_challenges(challenges, nthread=1, timeout=None):
    ...
    for i,c in enumerate(challenges):
        ...
        #print ch, s1, s2
        if ch!=c:
            print("ERROR the challenge returned is not correct:", "%d vs. %d" % (ch,c))
            total_errors+=1
            return 0
        if len(ss) != NSOLUTIONS+1:
            print ("ERROR returned an incorrect number of challenges")
            total_errors+=1
            return 0
        dd = {}
        for s in ss[1:]:
            if not test_hash(c,s):
                print ("ERROR the returned solution (%d) is not correct for the challenge (%d)" % (s,c))
                total_errors+=1
                return 0
